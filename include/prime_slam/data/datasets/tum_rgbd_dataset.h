...
/**
 * @brief Dataset for [TUM
 * RGBD](https://cvg.cit.tum.de/data/datasets/rgbd-dataset)
 */
class TUMRGBDDataset : public TUMRGBDDatasetBase {
  using Parameters = DefaultParameters<DataFormat::TUM>;
 ...
 private:
  struct RGBToDepthAssociation {
    size_t rgb_index;
    size_t depth_index;
  };

 private:
  static std::vector<RGBToDepthAssociation> CreateRGBToDepthAssociation(
      const std::vector<fs::directory_entry>& rgb_images_paths,
      const std::vector<fs::directory_entry>& depths_paths);

 private:
  std::vector<RGBToDepthAssociation> rgb_depth_associations_;
};
